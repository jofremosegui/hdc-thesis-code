...
class HdcGenericEncoder(nn.Module):
    def __init__(self, input_size, out_dimension, ngrams=7, dtype=torch.float32, device="cpu", vsa="HRR"):
        super().__init__()
        self.device = device
        self.vsa = vsa
        self.dtype = dtype
...
def main():
    INPUT_SHAPE = (1, 800, 7)
    MODEL_PATH = "results/HDC_HRR_UNDERSAMPLING/20250430_144301/model_20250430_182301.pt"  # Update this
    device = torch.device("cpu")

    model = HdcModel(input_size=INPUT_SHAPE[2], out_dimension=5000, ngrams=7, device=device, vsa="HRR")
