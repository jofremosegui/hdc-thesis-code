...
class HdcGenericEncoder(nn.Module):
    def __init__(self, input_size, out_dimension, ngrams=7, dtype=torch.complex64, device="cpu", vsa="FHRR"):
        super().__init__()
        self.device = device
        self.vsa = vsa
        self.dtype = dtype
...
def main():
    INPUT_SHAPE = (1, 800, 7)
    MODEL_PATH = "results/HDC_balanced_UNDERSAMPLING_FHRR/20250503_095435/model_20250507_030302.pt"
    device = torch.device("cpu")

    model = HdcModel(input_size=INPUT_SHAPE[2], out_dimension=5000, ngrams=7, device=device, vsa="FHRR")
